...
class ItemsService {
    ...
    async createOne(data, opts) {
        ...
        const primaryKey = await this.knex.transaction(async (trx) => {
            ...
            // "chronological" order
            const payloadAfterHooks = hooksResult.length > 0 ? hooksResult.reduce((val, acc) => lodash_1.merge(acc, val)) : payload;
            ...
            try {
                ...
                throw await translate_1.translateDatabaseError(err);
            }
    ...
    async readByQuery(query, opts) {
        ...
        let ast = await get_ast_from_query_1.default(this.collection, query, this.schema, {
            accountability: this.accountability,
        ...
        const records = await run_ast_1.default(ast, this.schema, {
            knex: this.knex,
    ...
    async upsert(data) {
        ...
        if (Array.isArray(data))
            return await this.upsertMany(data);
